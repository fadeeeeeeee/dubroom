...
// and Groq, no SDKs, no build step (same lightweight approach as docket's
// api/judge.js). Called by the browser right after an upload finishes.
//
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 2. Download the video file from Storage
    ...
    // 3. Transcribe with word-level timestamps via Groq Whisper
    ...
    // Group words into lines using pauses between them
    ...
    // 4. Moderate the transcript with Llama Guard
    ...
    // 5. Insert lines (character_id left null — see character-assignment
    //    step, not yet built) and publish
